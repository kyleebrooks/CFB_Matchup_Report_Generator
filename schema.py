...
SPEC: dict[str, dict] = {
    ...
    'voice_jobs': {
        'owner': 'service',
        'purpose': 'Episodes queued for an external VibeVoice studio to render, one row per job',
        'create': """
        ...
        'indexes': {
            'idx_voice_job_account': 'ADD KEY idx_voice_job_account (account_id, id)',
            'idx_voice_job_state':   'ADD KEY idx_voice_job_state (state, id)',
        },
    },
    'voice_workers': {
        'owner': 'service',
        'purpose': 'Known VibeVoice studios: last heartbeat and the voice catalog they advertise',
        'create': """
            CREATE TABLE IF NOT EXISTS voice_workers (
