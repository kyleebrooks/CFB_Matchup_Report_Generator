...
def generate_report():
    ...
    if not all([cfbd_api_key, search_api_key, google_cx, openai_api_key]):
    ...
    # If you want to see what we'll ask the model to read:
    ...
    prompt_intro = (
        ...
        f"You have a web_search tool enabled—use it to READ the linked articles directly and prefer their actual content over snippets. Do not report back on url's that you could not access, just use the context appropriately from the url's that you could access."
    ...
    # List the URLs explicitly (this is what web_search will read)
